...
exports.digest = function digest(options) {
  ...
  if (options.key != null) {
    console.warn("Deprecation Notice - Specifying the secret using `key` is no longer supported. Use `secret` instead.");
    secret = options.key;
  }
...
exports.hotp = function hotpGenerate(options) {
  ...
  if (options.length != null)
    console.warn("Deprecation Notice - Specifying token digits using `length` is no longer supported. Use `digits` instead.");
...
exports._counter = function _counter(options) {
  ...
  if (options.initial_time != null)
    console.warn("Deprecation Notice - Specifying the epoch using `initial_time` is no longer supported. Use `epoch` instead.");
...
exports.generateSecret = function generateSecret(options) {
  ...
  if (qr_codes) {
    console.warn(
      "Deprecation Notice - generateSecret() QR codes are deprecated and no longer supported. Please use your own QR code implementation."
    );
  ...
  if (google_auth_qr) {
    console.warn(
      "Deprecation Notice - generateSecret() Google Auth QR code is deprecated and no longer supported. Please use your own QR code implementation."
    );
    SecretKey.google_auth_qr =
      "https://chart.googleapis.com/chart?chs=166x166&chld=L|0&cht=qr&chl=" +
      encodeURIComponent(exports.otpauthURL({ secret: SecretKey.base32, label: name }));
  }
...
}, "Deprecation Notice - `generate_key()` is depreciated, please use `generateSecret()` instead.");
...
}, "Deprecation Notice - `generate_key_ascii()` is depreciated, please use `generateSecretASCII()` instead.");
